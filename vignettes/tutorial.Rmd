...
ggplot(data = cbind(my_gapminder, model), aes(x = model, y = lifeExp) ) +
  geom_point()
```
...
Neighbors <- rep(c("k = 2", "k = 5", "k = 10"), each = 30)
ggplot(data = as.data.frame(cbind(CVs, Neighbors)), aes(x = Neighbors, y = CVs, 
                                                        group = Neighbors)) +
  geom_boxplot()
mean_CV <- numeric(3)
```
